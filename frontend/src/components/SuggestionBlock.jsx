...
export default function SuggestionBlock({ step, formData, onSelect }) {
  ...
  const handleSuggest = async () => {
    ...
    try {
      ...
      if (safeList.length === 0) setErrorMsg('No suggestions came back — try again.');
    } catch (err) {
  ...
  return (
    <div className="suggest-row">
      <button className="suggest-btn" onClick={handleSuggest} disabled={isThinking}>
      ...
      <div className="chips">
        {suggestions.map((s, idx) => {
          ...
          return (
            <button key={idx} type="button" className="chip" onClick={() => onSelect(s.trim())}>
